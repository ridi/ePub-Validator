...
ImageValidator.prototype.validation = function() {
  ...
  if (isCover) {
    if (imgSize.width < config.cover_image_min_width ||
      ...
      report.add('IMG-301', path + '(' + imgSize.width + 'x' + imgSize.height + ')', 
                                  [config.cover_image_min_width, config.cover_image_min_height], 
                                  [config.cover_image_recommend_width, config.cover_image_recommend_height], null);
    }
  } else if (imgSize.width > config.content_image_max_width &&
             imgSize.height > config.content_image_max_height) {
    report.add('IMG-302', path + '(' + imgSize.width + 'x' + imgSize.height + ')', [config.content_image_max_width, config.content_image_max_height]);
  }
