...
ImageValidator.prototype.validation = function() {
  ...
  if (isCover) {
    if (imgSize.width < ImageValidator.COVER.MIN_WIDTH ||
      ...
      report.add('IMG-001', path, [ImageValidator.COVER.MIN_WIDTH, ImageValidator.COVER.MIN_HEIGHT], null,
                                  [ImageValidator.COVER.DEFAULT_WIDTH,  ImageValidator.COVER.DEFAULT_HEIGHT]);
    }
  } else if (imgSize.width > ImageValidator.CONTENT.MAX_WIDTH ||
             imgSize.height > ImageValidator.CONTENT.MAX_HEIGHT) {
    report.add('IMG-002', path, [ImageValidator.CONTENT.MAX_WIDTH, ImageValidator.CONTENT.MAX_HEIGHT]);
  }

  // 2. 이미지 파일 크기 검사
  //   - 저사양 기기에서 책을 열 수 없거나 랜더링할 수 없음
  var fileSizeInMB = Validator.Util.getFileSizeInMB(this.file);
  if (fileSizeInMB > ImageValidator.COMMON.MAX_FILE_SIZE_IN_MB) {
    var args = [ImageValidator.COMMON.MAX_FILE_SIZE_IN_MB];
    report.add('IMG-003', path, args, null, args);
  }
