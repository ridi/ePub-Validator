...
ImageValidator.prototype.validation = function() {
  ...
  if (isCover) {
    if (imgSize.width < ImageValidator.COVER.MIN_WIDTH ||
      ...
      log.info('표지 이미지의 권장 크기는 %dx%d입니다.', ImageValidator.COVER.DEFAULT_WIDTH, 
                                                 ImageValidator.COVER.DEFAULT_HEIGHT);
    }
  ...
  if (Validator.Util.getFileSizeInMB(this.file) > ImageValidator.COMMON.MAX_FILE_SIZE_IN_MB) {
    log.error('이미지 파일 크기가 %dMB를 초과했습니다. (%s)', ImageValidator.COMMON.MAX_FILE_SIZE_IN_MB, path);
  }
