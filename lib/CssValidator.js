...
var propertyAvail = (function() {
  var PropertyAvailDataSource = function() {
    ...
    var _listFile = fs.readFileSync(path.join(rootPath, 'db/property_avail.json'));
  ...
  if (result.stylesheet) {
    _.each(result.stylesheet.rules, function(rule) {
      ...
      _.each(rule.selectors, function(selector) {
        ...
        isImg = selector.match(/img/gi) !== undefined;
      });
      _.each(rule.declarations, function(declaration) {
        ...
        // 5. img에 position:relative 속성이 들어가 있는지 검사
        if (isImg && property == 'position' && value == 'relative') {
          report.add('CSS-303', path + '(' + location.line + '줄, ' + location.column + '열)');
        }
      });
    });
  }
