...
var shell         = require('shelljs'), /*epubcheck.jar 실행용*/
    fs            = require('fs'), 
    util          = require('util'),
...
EPubValidator.prototype.validation = function() {
  ...
  var command = 'java -Duser.country=KO -Duser.language=kr -jar ./lib/epubcheck/epubcheck.jar ' + this.file.replace(/([ '"])/gi, '\\$1') + ' -j ' + resultFile;
  ...
  if (result.publication === undefined) {
    ...
    if (result.publication.title !== null) {
      report.add('EPUB-001'/*도서 제목 정보*/, null, [result.publication.title]);
    }
    if (result.publication.creator.length > 0) {
      report.add('EPUB-002'/*저자 정보*/, null, [util.inspect(result.publication.creator)]);
    }
    if (result.publication.publisher !== null) {
      report.add('EPUB-003'/*출판 정보*/, null, [result.publication.publisher]);
    }
  }
