...
HtmlValidator.prototype.validation = function() {
  ...
  var fileSizeInKB = Validator.Util.getFileSizeInKB(this.file);
	if (fileSizeInKB > HtmlValidator.COMMON.MAX_FILE_SIZE_IN_KB) {
		log.error('HTML 파일 크기가 %dKB를 초과했습니다. (%s)', HtmlValidator.COMMON.MAX_FILE_SIZE_IN_KB, path);
	} else if (fileSizeInKB > HtmlValidator.COMMON.RECOMMENDED_FILE_SIZE_IN_KB) {
		log.warning('HTML 파일 크기가 %dKB를 초과했습니다. (%s)', HtmlValidator.COMMON.RECOMMENDED_FILE_SIZE_IN_KB, path);
	}  
};
