...
HtmlValidator.prototype.validation = function() {
  ...
  if (fileSizeInKB > config.html_file_max_size) {
    report.add('HTML-301'/*파일 크기 초과*/, path, [config.html_file_max_size], null, [config.html_file_recommend_file_size]);
  }
  ...
  function checker(/*Array*/nodes, /*Boolean*/isRoot) {
    ...
    _.each(nodes, function(node) {
      ...
      if (tag == 'body') {
        ...
        if (node.attribs !== undefined && (style = node.attribs['style']) !== undefined) {
          if (style.match(/background-color/gi) !== undefined) {
            ...
            report.add('HTML-303'/*body에서 background-color 속성이 발견됨*/, path + '(' + location.line + '줄, ' + location.col + '열)');
          }
        }
      }
      ...
      if ((result = tagAvail.atTag(tag))) {
        var location = findLocation(node.startIndex);
        report.add('HTML-201'/*OS 또는 앱에서 지원하지 않는 태그임*/, path + '(' + location.line + '줄, ' + location.col + '열)', [tag, result]);
      }
      ...
      if (node.children !== undefined) {
        ...
        if (node.children.length > config.child_nodes_limit) {
            var location = findLocation(node.startIndex);
            report.add('HTML-302'/*노드에 자식이 너무 많음*/, path + '(' + location.line + '줄, ' + location.col + '열, ' + tag + ')');
        }
  ...
  // 4. HTML에 body가 존재하는지
  if (hasBody === false) {
    report.add('HTML-401'/*body를 찾을 수 없음*/, path);
  }
